...
export const asyncSetUserInfo = createAsyncThunk(
  ...
  async (action: {
    ...
    return await action.firebaseApi.asyncSetUserInfo(
      action.userId,
      action.userInfo
    );
  }
...
export const asyncUpdateUserInfo = createAsyncThunk(
  ...
  async (action: {
    ...
    return await action.firebaseApi.asyncUpdateUserInfo(
      action.userId,
      action.userInfo
    );
  }
...
export const userSlice = createSlice({
  ...
  reducers: {
    ...
    setUserInfo: (
      ...
      action: PayloadAction<
        { value: UserInfo | null | undefined } & WithLoadingState
      >
    ) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(asyncGetUserInfo.rejected, (state) => {
        state.userInfo.value = undefined;
...
export const handleFollow =
  (firebaseApi: FirebaseApi, userId: string, targetUserId: string): AppThunk =>
  (dispatch, getState) => {
